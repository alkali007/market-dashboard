  ...
  const fetchInitialData = async () => {
    try {
      const [bRes, tRes] = await Promise.all([
        fetch('http://localhost:8081/dashboard/distribution?type=brand&limit=200'),
        fetch('http://localhost:8081/dashboard/distribution?type=category&limit=100')
      ]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [kpi, ps, dist, bp, tp, hm] = await Promise.all([
        fetch(`http://localhost:8081/dashboard/kpi?${qs}`).then(r => r.json()),
        fetch(`http://localhost:8081/dashboard/scatter?type=price_quantity&${qs}`).then(r => r.json()),
        fetch(`http://localhost:8081/dashboard/distribution?type=brand&${qs}`).then(r => r.json()),
        fetch(`http://localhost:8081/dashboard/brand?${qs}`).then(r => r.json()),
        fetch(`http://localhost:8081/dashboard/product-type?${qs}`).then(r => r.json()),
        fetch(`http://localhost:8081/dashboard/heatmap?${qs}`).then(r => r.json())
      ]);
